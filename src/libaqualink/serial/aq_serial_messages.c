...
bool process_aqualink_packet(unsigned char* rawPacket, unsigned int length)
{
	...
	MessageBus_Topic* simulator_topic = messagebus_find_topic_by_type(rs_controller_get_simulator_message_bus(), ((HardwareDeviceTypes)(rawPacket[DESTINATION_BYTE_LOCATION] & 0xFC)));
	if (0 != simulator_topic)
	{
		messagebus_topic_publish(simulator_topic, rawPacket, length);
		returnCode = true;
	}
	else
	{
